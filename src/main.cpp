...
#if defined(NDEBUG)
# error "Halfcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x13c00425b608b4eda84fcdcb06b584bea781e1c39fa3678ee61c6bb3f9530b16");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Halfcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Halfcoin Signed Message:\n";
...
{
    // Halfcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight == 2)  
    {
        nSubsidy = 136000000 * COIN;
    }

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Halfcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Halfcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Halfcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Halfcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Halfcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x33d4f1c486ec8dfb6e4d1489d3a3644941004676c15d7e66b2c168e92439f95f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04982fa47419a24c67e3cd28c8a9828d49816566dc0e2d1978c5e2bf7a1203a848722fd56a6ae55ec6eb625e9210f5a0b89306b850b7c7444d937bcc8770cb744b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1523549731;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1524521;

        if (fTestNet)
        {
            block.nTime    = 1523549798;
            block.nNonce   = 751066;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x43d859e96236775695aa5ce7950593399caa96f281fea75b94d924b30592e038"));
...
unsigned char pchMessageStart[4] = { 0xbb, 0x10, 0x11, 0xf4 }; // Halfcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HalfcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HalfcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HalfcoinMiner(CWallet *pwallet)
{
    printf("HalfcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("halfcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HalfcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HalfcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HalfcoinMiner, pwallet));
}
